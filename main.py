...
print("Song lyrics: ")
for i in songs.get_song_lyrics(songs.get_song_list()[song_number]):
    print(i)
print("")
'''

# spamming the chosen song
print("Spamming...")
while True:
    try:
        messages = songs.get_song_lyrics(songs.get_song_list()[song_index])
        for i in messages:
            # check if sucvat is given in settings.json
            if sucvat != "":
                sendMessage(token, channel, (f"<@{sucvat}>" + " " + i ))
            else:
                sendMessage(token, channel, i)
